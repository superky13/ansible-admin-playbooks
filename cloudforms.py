...
##
# Expanded the code put for by Guido and Josh P to parse ip addresses based on specific subnets
##
...
class CloudFormsInventory(object):
    ...
    def read_settings(self):
        ...
        if config.has_option('cloudforms', 'filter'):
            self.cloudforms_filter = config.get('cloudforms', 'filter')
        else:
            self.cloudforms_filter = None
        ...
        if self.args.debug:
            ...
            print("cloudforms_filter            = %s" % self.cloudforms_filter)
    ...
    def _get_hosts(self):
        ...
        while not last_page:
            offset = page * limit
            ret = self._get_json("%s/api/vms?offset=%s&limit=%s&expand=resources,tags,hosts,&attributes=ipaddresses&filter[]=%s" % (self.cloudforms_url, offset, limit, self.cloudforms_filter))
    ...
    def update_cache(self):
        ...
        for host in self._get_hosts():
            ...
            if 'ipaddresses' in host and host['ipaddresses'] and isinstance(host['ipaddresses'], list):
                validnetworks = re.compile("192.168|10.0") ##Change this to whatever subnet space you'd like to compare against
                ipv4addresses = filter(validnetworks.match, host['ipaddresses'])
                host['ansible_ssh_host'] = ipv4addresses[0]
